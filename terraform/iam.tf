...
# Additional policy for SSM parameters access
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          aws_ssm_parameter.binance_secret_key.arn
        ]
      }
    ]
  })
...
# Policy for CloudWatch logs (if needed by the application)
    ...
    Statement = [
      {
        ...
        Resource = "${aws_cloudwatch_log_group.delta_bot.arn}:*"
      }
    ]
  })
}